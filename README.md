...
3. Create a new folder in `/games/`. Its name is the short game name found on LinuxGSM Install page (for example `mc` for Minecraft).
...
If a game server requires post-install configuration, you can do it using a `post_install.sh` script. You can also use it to load configuration from environment variables. When asking user for input, make sure you also **support environment variables** for non-interactive installation.
